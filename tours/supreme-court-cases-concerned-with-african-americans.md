---
type: tour
category: history
read: false
---

# Tour — Supreme Court cases concerned with African-Americans

A foundations-first reading path. **Continue** jumps to your next unread node; each node has a Next ▶ link to flow through, and a Foundations box that checks what you still need to read first.

```dataviewjs
dv.view("_dv/tourpage", {list: "supreme-court-cases-concerned-with-african-americans", core: false, category: "history"})
```

