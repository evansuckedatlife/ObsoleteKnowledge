---
type: tour
category: literature
read: false
---

# Tour — Charles Dickens characters who aren’t protagonists

A foundations-first reading path. **Continue** jumps to your next unread node; each node has a Next ▶ link to flow through, and a Foundations box that checks what you still need to read first.

```dataviewjs
dv.view("_dv/tourpage", {list: "charles-dickens-characters-not-protagonists", core: false, category: "literature"})
```

